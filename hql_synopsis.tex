...
What if, many of the errors in software could be discovered by the compiler through clever\ab{Paragraphs don't link together, also casting has no meaning in Haskell}
...
try to reach a proof of concept level using a pure functional programming language called Haskell \cite{Haskell},
 where many desirable constructs for building robust and safe software is implemented.\\ \ab{I have a branch called rc2 where I have elaborated. In my opinion this is too short to give meaning}. 
...
The library is intended to be used for the following purposes: \ab{This is problem statement/learning goals, I do no think this should be included}
\begin{itemize}
...
state or note, because of the clear distinction made between pure functions and  monads that induce state (the \texttt{I/O} monad).\\
...
These will be ported to QuickCheck \cite{QuickCheck}, which is a combinator library for generating test cases in Haskell.\ab{This is not relevant? Also, we do not know if we have time for this.} 
...
A natural development of the project is to introduce a combinator library for composing contracts\ab{wouldn't this depend on our architecture whether it is feasible to do? Is it?}
from atomic constructs, as proposed by Peyton Jones et al. \cite{Pjes2000}.\\
