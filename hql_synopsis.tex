...
%Predefined and backtested trading algorithms are being used as a measure
%to lowering operational risk due as a result of human error, but can be exposed to flaws in software.\\
...
In this project, we will mainly work with closed-form functions for bonds
and the time value of money.\\

% Modularity
A functional language like Haskell lends itself well to the domain of finance as its
modularity permits us to easily `glue` computations together while non-strictness
will prevent us from enormous performance penalties\cite{hughes:matters-cj},

Moreover, \cite{composingcontracts} presents a combinator library for describing
financial contracts to further emphasize the possibilities.
...
Haskell data type containing the principal of the bond :
...
a consol bond could be defined using our {\tt CommonBond} from above:

\begin{center}
\tt
\begin{tabular}{lll}
data & Consol = Consol \{\\
      &\hspace{-1cm} bond   :: CommonBond,\\
...
As we see, the composite nature of Haskell's algebraic data types to enable new products
